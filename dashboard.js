...
document.addEventListener("DOMContentLoaded", function () {
    const waterBillIcon = document.getElementById("water-bill-icon");
    if (waterBillIcon) {
        waterBillIcon.addEventListener("click", function () {
            fetch("get-data.php")
                .then(response => response.text())
                .then(csvText => {
                    let rows = csvText.trim().split("\n");
                    let output = "<table border='1'><tr><th>नाम</th><th>बिल राशि</th></tr>";
                    for (let i = 1; i < rows.length; i++) {
                        let cols = rows[i].split(",");
                        output += `<tr><td>${cols[0]}</td><td>${cols[1]}</td></tr>`;
                    }
                    output += "</table>";
                    document.getElementById("data-display").innerHTML = output;
                })
                .catch(err => {
                    document.getElementById("data-display").innerHTML = "डेटा लोड में त्रुटि: " + err.message;
                });
        });
    }
});
